...
PERIODIC_SCREENSHOT_INTERVAL = 45  # Take screenshot every 45 seconds
...
def send_periodic_screenshot():
    ...
    try:
        ...
        if success:
            print(f"[PERIODIC] Sent periodic screenshot at {current_time}")
        return success
...
try:
    ...
    print("\n[EXIT] Screenshot system stopped by user.")
    print(f"[STATS] System ran for {int(time.time() - program_start_time)} seconds")
    print(f"[STATS] Approximately {int((time.time() - program_start_time) / PERIODIC_SCREENSHOT_INTERVAL)} screenshots taken")
